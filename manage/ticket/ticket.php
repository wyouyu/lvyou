...
if ( $_POST ) {
	...
	$table->beforeday = intval($_POST['beforeday']);
    ...
    $insert = array( 't_name', 't_partner', 't_origin', 't_price', 't_type',
         't_idnum', 't_status', 't_cate','type','start_num','team_price','team_start_num','deposit','beforeday','end_book_time','book_start_time','book_end_time','ticket_place','fanxian','t_sort','temaihui','temaisort');
    ...
    else
    {
        ...
        $p_insert = array('market_price','site_price','team_price','deposit','team_deposit','type','edit_time','ticket_id','tday','tdate','temaihui','temaisort');
    ...
    redirect( WEB_ROOT . '/manage/ticket/ticket.php?id='.$_POST['t_partner']);
}
else
{
    if($_GET['eid'])
...
include template('manage_ticket_ticket');