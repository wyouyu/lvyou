...
//$orderid = $_POST['id'];
if($_REQUEST['obday']&&$_REQUEST['oeday'])
{
	$ctime = strtotime($_REQUEST['obday']);
	$dtime = strtotime($_REQUEST['oeday']);
	$name = strval($_REQUEST['name']);
	
	$partner = $category= DB::LimitQuery('partner', array(
	'condition' => array('title'=>$name),
	...
	$condition2[] = "outtime >= '{$ctime}'";
	$condition2[] = "outtime <= '{$dtime}'";
	$condition2['type'] = 2;
	$condition2[] = 'status<>2';
	$condition2['partner_id'] = $partner['id'];
