...
class Utility 
{
    ...
    static public function Obption($a=array(), $v=null, $all=null,$images=null)
    {
        $option = null;
        if ( $all ){
            $selected = ($v) ? null : 'selected';
        ...
        foreach( $a AS $key=>$value )
        {
            ...
            if($images)
            {
                $selected = $key==$images ? 'selected' : null;
            }
            else
            {
                $selected = in_array($key, $v) ? 'selected' : null;
	...
	static function Post($data,$target)
	{
	...
	static function SendMess($phone,$content)
	{
		$flag = 0; 
        //要post的数据 
		$argv = array( 
			 'sn'=>'SDK-WSS-010-05922', ////替换成您自己的序列号
			 'pwd'=>strtoupper(md5(''.'')), //此处密码需要加密 加密方式为 md5(sn+password) 32位大写
			 'mobile'=>$phone,//手机号 多个用英文的逗号隔开 post理论没有长度限制.推荐群发一次小于等于10000个手机号
			 'content'=>urlencode( $content.'[爱米易旅游网]'),//短信内容
			 'ext'=>'',
