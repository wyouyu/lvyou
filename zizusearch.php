...
{
    $zizhuyou = DB::LimitQuery('zizhuyou', array(
        'condition' => array('partner_id2'=>$value['id']),
        'order' => 'ORDER BY id',
		'one'=>true
    ));

	$partners[$key]['price'] = $zizhuyou['price'];
	$partners[$key]['tese'] = strip_tags($zizhuyou['tese']);
}
//print_r($allinfo);
//热销排行
...
$current = 'zizhuyou';
include template('zizhuyou/zizhu_list');
?>